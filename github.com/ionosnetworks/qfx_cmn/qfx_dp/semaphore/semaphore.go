...
func (s *Semaphore) SetVal(semNum int, val int) error {
	_, _, err := syscall.Syscall6(syscall.SYS_SEMCTL, uintptr(s.semid),
		uintptr(semNum), uintptr(SETVAL), uintptr(val), uintptr(0), uintptr(0))
	return errnoErr(err)
}

func (s *Semaphore) Post(semNum int) error {
	post := semop{semNum: uint16(semNum), semOp: 1, semFlag: 0x0}
...
func (s *Semaphore) PostN(semNum int, num int16) error {
	post := semop{semNum: uint16(semNum), semOp: num, semFlag: 0x0}
	_, _, errno := syscall.Syscall(syscall.SYS_SEMOP, uintptr(s.semid),
		uintptr(unsafe.Pointer(&post)), uintptr(s.nsems))
	return errnoErr(errno)
}
...
func (s *Semaphore) Wait(semNum int) error {
	wait := semop{semNum: uint16(semNum), semOp: -1, semFlag: 0x0}
	_, _, errno := syscall.Syscall(syscall.SYS_SEMOP, uintptr(s.semid),
